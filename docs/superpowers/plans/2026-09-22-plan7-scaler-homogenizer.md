...
fn figure_mode_keeps_the_figure_bounding_box() {
  ...
  <text id="yl" x="8" y="45" style="font-size:6px;text-anchor:middle;{DV}" transform="rotate(-90,8,45)">value</text>
    ...
    assert!(close(after.x0, before.x0, 1e-6) && close(after.y0, before.y0, 1e-6), "top-left kept: {after:?} vs {before:?}");
    assert!(close(after.width(), before.width(), 1e-6) && close(after.height(), before.height(), 1e-6), "size kept: {after:?} vs {before:?}");
