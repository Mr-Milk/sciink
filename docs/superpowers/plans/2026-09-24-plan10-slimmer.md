# Slimmer Implementation Plan (Plan 10)
...
- Modify: `README.md` (Tools table, "Large documents"), `docs/spec/02-geometry-tools.md` (§B.3, new
  ...
  `Cargo.toml:3`, `Cargo.lock` (regenerated by the build), `tests/cli.rs:12,37`.
...
- [ ] **Step 3: Overview, DEVELOPING, CHANGELOG, version**
...
`CHANGELOG.md`: insert above `## 0.2.0 (unreleased)`:
...
(T10 appends the measured numbers to this bullet. If 0.2.0 has been tagged by then, the heading stays;
if not, the user folds the bullet into 0.2.0 at tag time.)

`Cargo.toml:3`: `version = "0.3.0"`. `tests/cli.rs:12` and `:37`: `sciink 0.3.0`. Run `cargo build` so
`Cargo.lock` follows.
...
```bash
/usr/bin/git add README.md docs/spec/02-geometry-tools.md docs/spec/00-overview.md docs/DEVELOPING.md CHANGELOG.md Cargo.toml Cargo.lock tests/cli.rs
/usr/bin/git commit -q -m "docs: Slimmer in README, spec, overview and DEVELOPING; version 0.3.0" -m "Documents the new tool and its deviations, replaces the 'link raster images' advice with the measured facts (Inkscape's round-trip cost is per stylesheet rule × element, images are irrelevant), and bumps the version." -m "Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>"
```
...
Append the measured numbers to the CHANGELOG 0.3.0 bullet as one sentence ("On a 52 MB manuscript:
...
off by default (significant digits); every default step rendering-exact. Ships as 0.3.0 unless the user
folds it into an untagged 0.2.0.
