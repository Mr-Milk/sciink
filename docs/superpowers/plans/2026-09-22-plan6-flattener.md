...
**Deviation (documented in Task 7):** the rgb comparison is exact and alpha is only checked on the top element, as upstream does (`inkex.Color` equality ignores alpha); the spec's "identical rgba" is read as upstream's.
