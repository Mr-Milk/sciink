...
- Document-wide walks are iterative (`Doc::descendants`, `Doc::children`). Recursion is allowed only where the depth is the nesting depth of groups, clip chains or `<use>` targets (`bbox`, `merge_clipmask`, `is_rectangle`), always carries a `depth` parameter, and stops with the safe default plus a warning past `ops::MAX_NEST = 64` (a Rust stack overflow cannot be caught; a hostile self-referencing clip or clone must not abort the process). **Amended by the final review:** a depth bound alone does not stop a clip that references itself from several children (the work grows exponentially with depth, and `merge_clipmask` clones as it descends so it cannot be memoised) — `merge_clipmask`, its rectangle tests and `unlink` also draw on a shared work budget `ops::MAX_STEPS = 10_000` per top-level call and stop with a warning when it is exhausted.
...
The whole-branch review (opus, range `484fdac..142efa7`) rated the branch "with fixes"; one fix wave landed as `f52232c`, `83838e2`, `364fd8b`, verified by a scoped re-review (all nine items addressed, no new breakage), CI green on every pushed head:

1. **F1 (Important)** `merge_clipmask` and `is_rectangle` recursed with a depth bound only; a clip referencing itself from two children is exponential. Now a shared work budget `ops::MAX_STEPS = 10_000` (`merge_rec`/`is_rect_rec` count steps; `is_rectangle_with` shares the merge's budget; `unlink` uses the same constant) with branching-cycle tests in `tests/ops_clip.rs` and `tests/ops_bbox.rs`. Global Constraints amended above.
2. **F2 (Important)** `combine_paths` deleted elements whose geometry it could not read ("skipped" but gone). Now only contributors are deleted, unreadable elements stay with a "left in place" warning, and a target without geometry or an out-of-range merge index refuses the merge.
3. Minors taken now: `gc_created_clips` fast path on an empty list; `fix_css_clipmask` pins only `none`/`url(#name)` (stylesheet-injection hardening; the test asserts the sheet, since the crafted attribute legitimately round-trips); `Doc::set_tag` bumps the stylesheet cache when renaming to or from `style`; `preferences_language` skips an `id="ui"` group without `language` and matches the id as a whole attribute; `Ctx::for_roots` documents that text outside the roots measures without kerning pairs or warnings; tests for `fill_is_url` and `ungroup(remove_text_clip = true)`; eight more deviation bullets in the spec; the oracle command in the README.
4. Parked with a ruling: `gradient_fixup` copies a shared `userSpaceOnUse` gradient once per fused element and never collects the copies — deferred to Plan 7 (the Scaler is the only caller that fuses many gradient-painted paths); file bloat until then. Every other deferred minor from the task reviews was accepted by the final review as not merge-blocking.

Execution rulings not visible in the tasks above: the Task 3 review stalled three times on sonnet and once on opus (runaway hand-tracing of test arithmetic) and was completed by a haiku checklist review; every later review used a bounded method (one focused `cargo test` run as the numeric evidence, no hand-tracing).
