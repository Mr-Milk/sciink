...
The whole-branch review (opus, range `2c3f496..5a5b25d`) rated the branch "with fixes"; one fix wave landed as ``af1c9da`, `56ced7a`, `afe607b`, `a5baa55``, verified by a scoped re-review (every item addressed, no new breakage) and by CI on every pushed head:

1. **F1/F2 (Important)** adding a template from a path without markers reported success and stored nothing, and applying a template with no stored markers silently removed the shapes' markers. Both are errors now ("The selected path has no markers to store." / "Template '<name>' has no markers stored."), documented as deviations (upstream stores an empty entry and strips silently).
2. **F3 (Important)** attributes copied from a marker into the store and from the store into a target document kept their prefixes (`sodipodi:nodetypes`) without declaring them at the destination. `Doc::ensure_prefix` declares a standard prefix (`KNOWN_NS`) on the root once and refuses unknown ones; `copy_attrs` uses it for every attribute copy and drops an attribute whose prefix cannot be declared. Tests round-trip a `sodipodi:` attribute through the store and into a document that did not declare it, and drop an unknown `foo:` one.
3. **F4 (Important)** the Add page captured the first shape in DOCUMENT order; upstream takes the first SELECTED shape — `Doc::selection_ordered` now, as the Scaler does.
4. **F5 (Important)** the spec's Storage paragraph still described the JSON store; it now describes the marker-element format and the `<inx dir>` fallback.
5. Minors taken now: an unchecked position removes a presentation-attribute marker too (`Doc::remove_style`); "Templates successfully updated!" only when something changed; `Store::markers` reads descendants, so a store re-saved by Inkscape (markers moved into `<defs>`) still loads; deviation bullets; a CHANGELOG line for Favorite Markers; the README pin example says `v0.1.0`.
6. Accepted with a ruling: CLI defaults differ from the `.inx` defaults (`template` 1, `emarker` false — upstream's argparse defaults; Inkscape always sends every parameter); `set`/`remove` share six lines; `url_id` requires the literal `url(#` (upstream-identical); only the first root `<defs>` is searched for reuse (upstream); the `INKSCAPE_PROFILE_DIR` branch of `store_path` is untested (`set_var` is unsafe in this edition).

Execution rulings not visible in the tasks above (all mirrored into the task text): the store is an SVG document written with the crate's DOM instead of the spec's JSON (no serde; the DOM round-trips prefixed attributes; the file opens in Inkscape); the hidden `--store <path>` parameter exists for tests; `tests/cli.rs`'s unimplemented-tool test was retired when the last tool landed; the packaging check builds, packages, then tests the zip; `const NS` in the test file carried `#[allow(dead_code)]` for one task; clap kebab-cases multi-word flags, so `custom_name`/`template_name`/`template_rem` carry explicit `long = "…"`; the `markers` and `new_marker` test helpers are named-lifetime fns (a closure cannot name the returned node's lifetimes), `new_marker` module-level over the input's id set; the Markers page never writes the store, so the oracle's cleanup is tolerant.
